...
{
public:
  site (char const *newkey) {key = new char[strlen (newkey+1)];strcpy (key,newkey);};
  ~site () {if (key) delete key;};
  char * key;
};
...
public:
   ...
   */
  virtual const char *Canonical () {return canonical;};
  /* The basename - without extention 
   * i.e. package-1
   */
  virtual const char *Base () {return base;};
  ...
  virtual char const *Cached () {return cached;};
