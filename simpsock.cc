...
SimpleSocket::SimpleSocket (const char *hostname, int port)
{
  ...
  if (sscanf (hostname, "%d.%d.%d.%d", &i1, &i2, &i3, &i4) == 4)
  ...
  else
    {
      ...
      if (!he)
	{
          Log (LOG_PLAIN) << "Can't resolve '" << hostname << "'" << endLog;
	  return;
	}
  ...
  if (s == INVALID_SOCKET)
    {
      Log (LOG_PLAIN) << "Can't create socket, " << WSAGetLastError () << endLog;
      return;
    }
  ...
  if (connect (s, (sockaddr *) & name, sizeof (name)))
    {
      Log (LOG_PLAIN) << "Can't connect to " << hostname << ":" << port << endLog;
      closesocket (s);
      s = INVALID_SOCKET;
      return;
    }
